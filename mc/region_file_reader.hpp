...
private:
	...
	T read_value(byte_stream &stream) {
        ...
        stream >> value; // retrieve value
        return value;
	}

	template <class T>
    T read_long_value(byte_stream &stream){ /* Use for reading long values if standard does not work */
        T value=0;

        int temp;
        for (int i=0;i<sizeof(T)/4;i++){
            stream>>temp; //get 4 bytes
            value = (value<<32)+temp; //shift and add on the rest
        }
